...
def get_feature_map_size(model):
    ...
    x = model.get_layer(index=-1).output.shape[1]
    y = model.get_layer(index=-1).output.shape[2]
...
def project_embeddings(
    ...
    projected_embeddings = TimeDistributed(layers.Dense(units=projection_dims))(embeddings)
    for _ in range(num_projection_layers):
        x = TimeDistributed(tf.nn.gelu(projected_embeddings))
        x = TimeDistributed(layers.Dense(projection_dims))(x)
...
def classifier_ZSL(base_layers, input_rois, num_rois, num_projection_layers, projection_dims, dropout_rate, nb_classes = 21, trainable=False):
